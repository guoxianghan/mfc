...
#pragma warning restore 649
        ...
        void ReadEquipLine()
        {
            ...
            foreach (var items in agvroutegroup)
            {
                ...
                {
                    #region MyRegion
                    ne.AddField(item.LWCS.Trim(), "0");// 1出拉空轮; 2入拉满轮
                    ne.AddField(item.WAStatus.Trim(), "0");//是否已调度AGV 1，已接收滚筒出料请求；2，已接收入料至滚筒请求 光电
                    ne.AddField(item.AGVDisStatus.Trim(), "0");//地面滚筒请求 AGVDisStatus
                    ne.AddField(item.LStatus.Trim(), "0");//读取消标记1出拉空轮; 2入拉满轮
        ...
        void ParserNe(List<EquipConfigerEntity> list, Neutrino ne)
        {
            ...
            foreach (var item in list)
            {
                ...
                agvrunning = ne.GetInt(item.WAStatus.Trim());//是否已调度AGV 1，已接收滚筒出料请求；2，已接收入料至滚筒请求 光电 w400
                recive = ne.GetInt(item.AGVDisStatus.Trim());//地面滚筒请求 AGVDisStatus 0没料,1,2有料可以创建任务 w500
                cancel = ne.GetInt(item.LStatus.Trim());//读取消标记1出拉空轮; 2入拉满轮
                iscancel = ne.GetInt(item.EStatus2.Trim());//是否可取消标记1出拉空轮; 2入拉满轮
                                                           /// <summary>
                                                           /// 0初始化EquipTask,
                                                           /// 1创建AGVTask和龙门Task,
                                                           /// 2正在抓取,
                                                           ...
                                                           /// 14无法取消,回复取消完毕
                                                           /// </summary>
                var equiptsks = this.BusinessLogic.EquipTaskProvider.GetEquipTaskEntitySqlWhere($"[EquipContollerId]='{item.ControlID}' and Status IN(0,1,2,3,4,5,6,9,10,13,14)");
                ...
                if (cancel != 0)
                {
                    ...
                    if (equiptsk != null && (equiptsk.Status == 0 || equiptsk.Status == 10))
                    {
                        equiptsk.Status = 11;//可取消
                        this.BusinessLogic.EquipTaskProvider.UpdateStatus(11, null, equiptsk.Id);
                    }
                    if (equiptsk != null && (equiptsk.Status != 0 && equiptsk.Status != 10))
                    {
                        equiptsk.Status = 13;//不可取消
                        this.BusinessLogic.EquipTaskProvider.UpdateStatus(13, null, equiptsk.Id);
                    }
                    if (equiptsk != null)
                    {
        ...
        void WriteEquipLine()
        {
            ...
            var equiptsks = this.BusinessLogic.EquipTaskViewProvider.GetEquipTaskViewEntities($"[PLCNO]='{this.PLCNo}' and Status IN(0,1,2,3,4,5,6,9,10,13,14)", null);
            ...
            if (equiptsks == null) equiptsks = new List<EquipTaskViewEntity>();
            if (_EquipCmdList == null)
                _EquipCmdList = this.BusinessLogic.EquipConfigerProvider.EquipConfigers.FindAll(x => x.PLCNo == PLCNo);
            ...
            var manlun = equiptsks.FindAll(x => x.TaskType == 2);
            var konggroup = kong.GroupBy(x => x.AGVRoute.Trim());
            #region 创建拉空轮的任务 
